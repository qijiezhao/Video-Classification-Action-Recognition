...
if args.dataset=='ucf101':
    with open('../data/ucf101/metadata/alllist.txt','r')as fp:
        lines=fp.readlines()
        for line in lines:
            eval_video_list.append(line.strip().split('.')[0].split('/')[-1])
elif args.dataset=='hmdb51':
    with open('data/hmdb51_all_list.txt','r')as fp:
        lines=fp.readlines()
        for line in lines:
            eval_video_list.append(line.strip().split(' ')[0].split('/')[-1])
...
def eval_video(video):
    ...
    #label = video[1]
    vid = video
    save_path=os.path.join('../data/ucf101/feature_rgb_pool/',vid+'.npy')
    ...
    for tick in frame_ticks:
        if args.modality == 'rgb':
            name = '{}{:04d}.jpg'.format(args.rgb_prefix, tick)
        ...
        if args.modality == 'flow':
            ...
            for idx in frame_idx:
                x_name = '{}{:04d}.jpg'.format(args.flow_x_prefix, idx)
                y_name = '{}{:04d}.jpg'.format(args.flow_y_prefix, idx)
    ...
    scores_return=np.array(frame_scores)
    feature_save=scores_return.mean(1).reshape(25,1024)
    np.save(os.path.join('../data/ucf101/feature_rgb_pool/',vid+'.npy'),feature_save)
    print 'video {} done'.format(vid)
    sys.stdin.flush()
    return scores_return#, label
